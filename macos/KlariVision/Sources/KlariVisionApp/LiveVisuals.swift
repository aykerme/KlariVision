...
struct LiveWebPitchGraph: NSViewRepresentable {
    ...
    (()=>{
      ...
      const current=()=>s.now+(s.animating?(performance.now()-s.received)/1000:0);
      const draw=()=>{
        resize();const w=c.clientWidth,h=c.clientHeight,L=86,R=18,T=16,B=28,cw=Math.max(1,w-L-R),ch=Math.max(1,h-T-B),now=current(),start=now-s.visibleDuration,high=s.verticalCenter+s.verticalSpan/2;
