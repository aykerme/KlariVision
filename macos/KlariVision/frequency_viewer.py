...
HTML_TEMPLATE = """<!doctype html><html lang="tr"><meta charset="utf-8">
<title>KlariVision __VIEWER_VERSION__ — Duyulan frekans</title>
<style>
:root{
...
</style><body class="__AUDIO_ONLY_CLASS__"><main>
<h1>KlariVision __VIEWER_VERSION__ · Pitch Konturu</h1><a id="new-recording" class="new-recording" href="/">Yeni Video / Ses Seç</a>
<p>Pitch eğrisi pYIN'in ölçtüğü fiziksel frekanstır (Hz). Türk Müziği (Sol Klarnet) ekseni koma miktarını gösterir.</p>__ANALYSIS_STATUS_HTML__
...
<div id="workspace" class="workspace __AUDIO_ONLY_CLASS__"><div id="media-panel" class="media">__MEDIA_HTML____MEDIA_HANDLES__</div>
...
const audioOnly=__AUDIO_ONLY_JSON__;
const frames=__FRAMES_JSON__;
const scaleLabels=__SCALE_LABELS_JSON__;
const turkishReference=__TURKISH_REF_JSON__;
const makamDefaults=__MAKAM_DEFAULTS_JSON__;
...
function tick(){if(loopEnabled&&!media.paused&&(media.currentTime||0)>=loopB)restartLoopAtA();advanceClock();if(followPlayback)centerOnPlayhead();draw();requestAnimationFrame(tick)}
...
organisePracticeControls();addAppearanceSettings();document.getElementById('minus').onclick=()=>setWindow(windowSeconds*1.35);document.getElementById('plus').onclick=()=>setWindow(windowSeconds/1.35);document.getElementById('reset').onclick=()=>{media.currentTime=0;followPlayback=true;centerOnPlayhead();draw()};winInput.onchange=()=>setWindow(Number(winInput.value));updateMakamStatus();
...
def build_frequency_viewer(
    ...
    *,
    ...
    turkish_ref_list = [
        record.__dict__ | {"display_notation": record.display_notation, "octave_label": record.octave_label}
        for record in turkish_reference
    ]

    html_content = (
        HTML_TEMPLATE.replace("__VIEWER_VERSION__", VIEWER_VERSION)
        .replace("__AUDIO_ONLY_CLASS__", "audio-only" if is_audio_only else "")
        .replace("__ANALYSIS_STATUS_HTML__", status)
        .replace("__MEDIA_HTML__", media)
        .replace("__MEDIA_HANDLES__", media_handles)
        .replace("__AUDIO_ONLY_JSON__", json.dumps(is_audio_only))
        .replace("__FRAMES_JSON__", json.dumps(frames, ensure_ascii=False, separators=(",", ":")))
        .replace("__SCALE_LABELS_JSON__", json.dumps(SCALE_LABELS, ensure_ascii=False, separators=(",", ":")))
        .replace("__TURKISH_REF_JSON__", json.dumps(turkish_ref_list, ensure_ascii=False, separators=(",", ":")))
        .replace("__MAKAM_DEFAULTS_JSON__", json.dumps(MAKAM_DEFAULT_INTERVALS, ensure_ascii=False, separators=(",", ":")))
    )
