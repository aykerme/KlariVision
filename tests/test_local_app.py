...
from klarivision.pitch.cpp_engine import ENGINES as CPP_ENGINES, OFFLINE_TRACK_REVISION
...
def test_cpp_study_cache_is_separate_for_each_user_pitch_engine(tmp_path, monkeypatch) -> None:
    source = tmp_path / "icra.wav"
    source.write_bytes(b"portable-pcm-source")
    monkeypatch.setattr("klarivision.local_app.PROJECT_ROOT", tmp_path)
    monkeypatch.setattr("klarivision.local_app.IMPORTS_DIR", tmp_path / "data" / "imports")
    monkeypatch.setattr("klarivision.local_app.AUDIO_DIR", tmp_path / "data" / "audio")
    monkeypatch.setattr("klarivision.local_app.OUTPUTS_DIR", tmp_path / "outputs")
    ...
    def fake_extract(_wav, engine, destination):
        calls.append((engine, destination))
        destination.parent.mkdir(parents=True, exist_ok=True)
        destination.write_text('{"frames":[]}', encoding="utf-8")
    ...
    monkeypatch.setattr("klarivision.local_app._to_wav", fake_to_wav)
    monkeypatch.setattr("klarivision.local_app.extract_cpp_pitch", fake_extract)
    monkeypatch.setattr("klarivision.local_app.build_frequency_viewer", fake_build_frequency_viewer)
    ...
    assert {
        path.name for _, path in calls
    } == {
        f"icra-{_file_signature(source)}.{engine}.offline_track_v1.{OFFLINE_TRACK_REVISION}.json"
