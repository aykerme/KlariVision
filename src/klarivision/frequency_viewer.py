...
function separatePanels(active){{const other=active===mediaPanel?chartPanel:mediaPanel;if(!panelsOverlap(active,other))return;const gap=16,current={{left:active.offsetLeft,top:active.offsetTop}},candidates=[{{left:other.offsetLeft-active.offsetWidth-gap,top:current.top}},{{left:other.offsetLeft+other.offsetWidth+gap,top:current.top}},{{left:current.left,top:other.offsetTop-active.offsetHeight-gap}},{{left:current.left,top:other.offsetTop+other.offsetHeight+gap}}].filter(candidate=>candidate.left>=0&&candidate.top>=0&&!((candidate.left<other.offsetLeft+other.offsetWidth)&&(candidate.left+active.offsetWidth>other.offsetLeft)&&(candidate.top<other.offsetTop+other.offsetHeight)&&(candidate.top+active.offsetHeight>other.offsetTop)));candidates.sort((a,b)=>(a.left-current.left)**2+(a.top-current.top)**2-(b.left-current.left)**2-(b.top-current.top)**2);const target=candidates[0]||{{left:current.left,top:other.offsetTop+other.offsetHeight+gap}};active.style.left=`${{target.left}}px`;active.style.top=`${{target.top}}px`;keepWorkspaceTall(active)}}
