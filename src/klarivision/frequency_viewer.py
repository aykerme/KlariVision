...
function followValues(){{const time=media.currentTime||0,[from,to]=frameRangeFor(time-.25,time+.25),values=[];for(let i=from;i<to;i++)values.push(cents(frames[i].hz));return values}}
...
function updateScrollbars(){{timeScroll.min=0;timeScroll.max=Math.max(1,Math.round(duration*1000));timeScroll.value=Math.round((media.currentTime||0)*1000);if(!frames.length)return;if(!frameCentsBounds){{let min=Infinity,max=-Infinity;for(const p of frames){{const c=cents(p.hz);if(c<min)min=c;if(c>max)max=c}}frameCentsBounds=[min,max]}}verticalScroll.min=Math.floor(frameCentsBounds[0]-verticalSpan/2);verticalScroll.max=Math.ceil(frameCentsBounds[1]+verticalSpan/2);verticalScroll.value=Math.round(verticalCenter)}}
