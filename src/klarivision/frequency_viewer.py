...
VIEWER_VERSION = "0.4.5-makam-dogrulama"
...
<label>Eksen <select id="scale-mode"><option value="major">Majör</option><option value="minor">Minör</option><option value="nihavent">Nihavend</option><option value="kurdi">Kürdi</option><option value="ussak">Uşşak</option><option value="turkish">Türk Müziği · Sol Klarnet</option></select></label><button id="makam-settings-open" type="button">Makam ayarları</button><span id="makam-status" class="countdown-status"></span>
...
const makamSettingsDialog=document.getElementById('makam-settings'),makamSettingsOpen=document.getElementById('makam-settings-open'),makamSettingsMode=document.getElementById('makam-settings-mode'),makamSettingsGrid=document.getElementById('makam-settings-grid'),makamSettingsTotal=document.getElementById('makam-settings-total'),makamSettingsApply=document.getElementById('makam-settings-apply'),makamSettingsReset=document.getElementById('makam-settings-reset');
const makamStatus=document.getElementById('makam-status');
...
const makamNames={{nihavent:'Nihavend',kurdi:'Kürdi',ussak:'Uşşak'}};
...
function makamNotes(mode,labelMode){{const solClarinetTonic=Number(tonicInput.value),soundingTonic=(solClarinetTonic+7)%12,tonicMidi=60+soundingTonic,names=scaleLabels[labelMode][String(solClarinetTonic)],steps=[0];for(const interval of makamIntervals[mode])steps.push(steps.at(-1)+interval);const result=[];for(let octave=-3;octave<=3;octave++){{for(let degree=0;degree<7;degree++){{const hz=440*Math.pow(2,(tonicMidi-69)/12)*Math.pow(2,octave)*Math.pow(2,steps[degree]/53);result.push([`${{names[degree]}}${{Math.floor(tonicMidi/12)-1+octave}}`,hz])}}}}return result}}
...
function scaleNotes(){{if(scaleMode.value==='turkish')return turkishNotes();if(scaleMode.value==='nihavent')return nihaventNotes();if(scaleMode.value==='kurdi')return kurdiNotes();if(scaleMode.value==='ussak')return ussakNotes();return notesForMode(scaleMode.value,scaleMode.value==='major'?[0,2,4,5,7,9,11]:[0,2,3,5,7,8,10])}}
...
function updateMakamStatus(){{if(!makamIntervals[scaleMode.value]){{makamStatus.textContent='';return}}makamStatus.textContent=`${{makamNames[scaleMode.value]}}: ${{makamIntervals[scaleMode.value].join('–')}} = 53 koma`}}
...
document.getElementById('minus').onclick=()=>setWindow(windowSeconds*1.35);document.getElementById('plus').onclick=()=>setWindow(windowSeconds/1.35);document.getElementById('reset').onclick=()=>{{media.currentTime=0;followPlayback=true;centerOnPlayhead();draw()}};winInput.onchange=()=>setWindow(Number(winInput.value));updateMakamStatus();
...
scaleMode.onchange=()=>{{tonicInput.disabled=scaleMode.value==='turkish';updateMakamStatus();draw()}};tonicInput.onchange=()=>draw();
