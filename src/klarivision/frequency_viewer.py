...
<style>
:root{{--music-font:'KlariVision Bravura','Apple Symbols','Noto Music',serif}}@font-face{{font-family:'KlariVision Bravura';src:url('../assets/fonts/BravuraText.otf') format('opentype');font-display:block}}
...
<p>Pitch eğrisi pYIN'in ölçtüğü fiziksel frekanstır (Hz). Türk Müziği (Sol Klarnet) ekseni, referans tablosundaki AEU koma işaretlerini ve alt/üst oktavlarını gösterir; ölçülen eğri değiştirilmez.</p>
...
const turkishReference={json.dumps([record.__dict__ | {"accidental_notation": record.accidental_notation, "octave_label": record.octave_label} for record in turkish_reference], ensure_ascii=False, separators=(',', ':'))};
...
function turkishNotes(){{return turkishReference.map(note=>[`${{note.accidental_notation}} · ${{note.turkish_name}} (${{note.octave_label}})`,note.frequency_hz])}}
...
for(const [name,hz] of scaleNotes()){{const yy=y(hz);if(yy<marginTop-5||yy>h-bottom+5)continue;ctx.beginPath();ctx.moveTo(left,yy);ctx.lineTo(w-right,yy);ctx.stroke();ctx.fillStyle='#3d4854';ctx.textAlign='right';ctx.font=scaleMode.value==='turkish'?"13px 'KlariVision Bravura','Apple Symbols',serif":'11px system-ui';ctx.fillText(`${{name}}  (${{hz.toFixed(2)}} Hz)`,left-9,yy+4)}}
...
document.fonts?.load("13px 'KlariVision Bravura'").then(()=>draw());document.getElementById('minus').onclick=()=>setWindow(windowSeconds*1.35);document.getElementById('plus').onclick=()=>setWindow(windowSeconds/1.35);document.getElementById('reset').onclick=()=>{{media.currentTime=0;followPlayback=true;centerOnPlayhead();draw()}};winInput.onchange=()=>setWindow(Number(winInput.value));
