...
<p>Pitch eğrisi pYIN'in ölçtüğü fiziksel frekanstır (Hz). Sol klarnet seçeneği yalnızca görünümü −22 koma kaydırır; kaynak pitch verisi değişmez.</p>
...
<label><input id="sol-clarinet" type="checkbox"> Sol klarnet (−22 koma)</label>
...
const scaleMode=document.getElementById('scale-mode'),tonicInput=document.getElementById('tonic'),solClarinetInput=document.getElementById('sol-clarinet');
...
const SOL_CLARINET_FACTOR=Math.pow(2,-22/53);function displayHz(hz){{return solClarinetInput.checked?hz*SOL_CLARINET_FACTOR:hz}}function cents(hz){{return 1200*Math.log2(hz/440)}}
...
function visibleValues(){{return frames.filter(p=>p.t>=viewStart&&p.t<=viewStart+windowSeconds).map(p=>cents(displayHz(p.hz)))}}
...
ctx.save();ctx.beginPath();ctx.rect(left,marginTop,cw,ch);ctx.clip();ctx.strokeStyle='#111820';ctx.lineWidth=1.7;ctx.lineJoin='round';ctx.lineCap='round';ctx.beginPath();let previous=null;for(const p of frames){{if(p.t<viewStart-.05||p.t>viewStart+windowSeconds+.05)continue;const xx=x(p.t),yy=y(displayHz(p.hz));if(!previous||p.t-previous.t>.030)ctx.moveTo(xx,yy);else ctx.lineTo(xx,yy);previous=p}}ctx.stroke();
...
scaleMode.onchange=()=>draw();tonicInput.onchange=()=>draw();solClarinetInput.onchange=()=>{{verticalReady=false;draw()}};
