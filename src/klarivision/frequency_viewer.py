...
function normalizeGraphAppearance(value){{return {{pitchHex:validGraphColor(value?.pitchHex)||defaultGraphAppearance.pitchHex,noteGuideHex:validGraphColor(value?.noteGuideHex)||defaultGraphAppearance.noteGuideHex}}}}
...
function setGraphAppearance(value,{{persist=true,notify=true}}={{}}){{graphAppearance=normalizeGraphAppearance(value);if(persist)localStorage.setItem(graphAppearanceKey,JSON.stringify(graphAppearance));if(graphAppearanceInputs){{graphAppearanceInputs.pitch.value=graphAppearance.pitchHex;graphAppearanceInputs.noteGuide.value=graphAppearance.noteGuideHex}}if(notify)window.webkit?.messageHandlers?.graphAppearance?.postMessage(graphAppearance);draw();return graphAppearance}}
function chartPalette(){{if(document.body.classList.contains('theme-studio'))return {{background:'#20262d',grid:'#3b4651',axis:'#bdcad5',label:'#e7f3f7',curve:graphAppearance.pitchHex,guide:graphAppearance.noteGuideHex,frame:'#657888',loop:'rgba(51,184,199,.23)',playhead:'#d99aff'}};if(document.body.classList.contains('theme-classic'))return {{background:'#fffaf1',grid:'#eadfce',axis:'#a38b69',label:'#263647',curve:graphAppearance.pitchHex,guide:graphAppearance.noteGuideHex,frame:'#c7a66d',loop:'rgba(204,162,81,.20)',playhead:'#9a6ab0'}};return {{background:'#fff',grid:'#e2e6eb',axis:'#edf0f3',label:'#3d4854',curve:graphAppearance.pitchHex,guide:graphAppearance.noteGuideHex,frame:'#aeb8c3',loop:'rgba(112,181,235,.20)',playhead:'#7755b8'}}}}
...
window.klariVisionStudyViewer={{
    ...
    snapshot(){{
        ...
        return {{time,duration:Number.isFinite(media.duration)?media.duration:duration,ready:mediaReady,isPlaying:!media.paused,rate:Number(media.playbackRate)||1,loopEnabled,loopA,loopB,theme:activeAppearance(),followsCurve:verticalFollowInput.checked,frequency:nearest&&delta<=.15?nearest.hz:null,scale:scaleMode.value,tonic:Number(tonicInput.value),intervals:makamIntervals[scaleMode.value]||null}};
    }},
    command(value){{
...
function tick(){{if(loopEnabled&&!media.paused&&(media.currentTime||0)>=loopB)restartLoopAtA();advanceClock();if(followPlayback)centerOnPlayhead();if(timeReadout){{const text=`${{formatClock(playbackTime())}} / ${{formatClock(duration)}}`;if(timeReadout.textContent!==text)timeReadout.textContent=text}}draw();requestAnimationFrame(tick)}}
