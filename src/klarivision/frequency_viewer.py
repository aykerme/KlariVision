...
function restartLoopAtA(){{const start=Math.max(0,Math.min(loopA,Math.max(0,duration-.001)));media.currentTime=start;if(!media.paused)media.play().catch(()=>{{}})}}
...
setAButton.onclick=()=>{{loopA=Math.max(0,Math.min(media.currentTime||0,duration));if(loopA>=loopB-.02){{loopB=duration;loopBManual=false}}updateLoopButtons();draw()}};setBButton.onclick=()=>{{loopB=Math.max(loopA+.02,Math.min(media.currentTime||0,duration));loopBManual=true;updateLoopButtons();draw()}};loopButton.onclick=()=>{{if(!loopEnabled&&loopB-loopA<.02){{countdownStatus.textContent='Loop için A ile B arasında en az 0,02 sn olmalı.';return}}loopEnabled=!loopEnabled;if(loopEnabled){{restartLoopAtA();countdownBypass=true;media.play()}}else if(media.currentTime<duration){{countdownBypass=true;media.play()}}updateLoopButtons();draw()}};
