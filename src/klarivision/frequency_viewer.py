...
const graphAppearanceKey='klarivision-graph-appearance-v1',defaultGraphAppearance={{pitchHex:'#0A84FF',noteGuideHex:'#8E8E93'}};
...
function normalizeGraphAppearance(value){{return {{pitchHex:validGraphColor(value?.pitchHex)||defaultGraphAppearance.pitchHex,noteGuideHex:validGraphColor(value?.noteGuideHex)||defaultGraphAppearance.noteGuideHex,micHex:validGraphColor(value?.micHex)||defaultGraphAppearance.micHex}}}}
...
function setGraphAppearance(value,{{persist=true,notify=true}}={{}}){{graphAppearance=normalizeGraphAppearance(value);if(persist)localStorage.setItem(graphAppearanceKey,JSON.stringify(graphAppearance));if(graphAppearanceInputs){{graphAppearanceInputs.pitch.value=graphAppearance.pitchHex;graphAppearanceInputs.noteGuide.value=graphAppearance.noteGuideHex;graphAppearanceInputs.mic.value=graphAppearance.micHex}}if(notify)window.webkit?.messageHandlers?.graphAppearance?.postMessage(graphAppearance);draw();return graphAppearance}}
function chartPalette(){{if(document.body.classList.contains('theme-studio'))return {{background:'#20262d',grid:'#3b4651',axis:'#bdcad5',label:'#e7f3f7',curve:graphAppearance.pitchHex,guide:graphAppearance.noteGuideHex,mic:graphAppearance.micHex,frame:'#657888',loop:'rgba(51,184,199,.23)',playhead:'#d99aff'}};if(document.body.classList.contains('theme-classic'))return {{background:'#fffaf1',grid:'#eadfce',axis:'#a38b69',label:'#263647',curve:graphAppearance.pitchHex,guide:graphAppearance.noteGuideHex,mic:graphAppearance.micHex,frame:'#c7a66d',loop:'rgba(204,162,81,.20)',playhead:'#9a6ab0'}};return {{background:'#fff',grid:'#e2e6eb',axis:'#edf0f3',label:'#3d4854',curve:graphAppearance.pitchHex,guide:graphAppearance.noteGuideHex,mic:graphAppearance.micHex,frame:'#aeb8c3',loop:'rgba(112,181,235,.20)',playhead:'#7755b8'}}}}
...
window.klariVisionStudyViewer={{
    ...
    setGraphAppearance,
    settingsSnapshot(){{
        return {{theme:activeAppearance(),scale:scaleMode.value,tonic:Number(tonicInput.value),countdown:Number(countdownInput.value)||0,intervals:makamIntervals,graphAppearance}};
    }},
    applySettings(value){{
...
for(const [name,hz] of scaleNotes()){{const yy=y(hz);if(yy<marginTop-5||yy>h-bottom+5)continue;ctx.beginPath();ctx.moveTo(left,yy);ctx.lineTo(w-right,yy);ctx.strokeStyle=colorWithAlpha(palette.guide,.42);ctx.stroke();ctx.fillStyle=colorWithAlpha(palette.guide,.88);ctx.textAlign='right';ctx.font='11px system-ui';ctx.fillText(`${{name}}  (${{hz.toFixed(2)}} Hz)`,left-9,yy+4)}}
