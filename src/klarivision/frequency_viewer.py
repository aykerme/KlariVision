...
from klarivision.i18n import (
    ENGINE_LABELS as _ENGINE_LABELS,
    engine_label as _engine_label,
    translate as _translate,
    translate_js as _translate_js,
)
...
def prepare_display_frames(payload: dict[str, object]) -> list[dict[str, float]]:
    ...
    candidates = [
        ...
        # The portable Study engines explicitly support 80–1500 Hz.  Dropping
        # values below 112 Hz made valid low-register clarinet notes vanish
        # only in the user-visible curve, while the raw C++ track was correct.
        and float(frame["frequency_hz"]) >= 80
...
<p>{_translate("viewer-description", lang)}</p>{status}
<div class="layout-tools"><label>{_translate("viewer-layout-label", lang)} <select id="layout-mode"><option value="stacked">{_translate("viewer-layout-stacked", lang)}</option><option value="side">{_translate("viewer-layout-side", lang)}</option><option value="side-right">{_translate("viewer-layout-side-right", lang)}</option></select></label></div>
...
<button id="minus">− {_translate("viewer-zoom-time-word", lang)}</button><button id="plus">+ {_translate("viewer-zoom-time-word", lang)}</button>
<label>{_translate("viewer-visible-duration-label", lang)} <input id="window" type="number" min="2" max="60" step="1" value="12"> {_translate("viewer-seconds-unit", lang)}</label>
<button id="vertical-out">− {_translate("viewer-vertical-word", lang)}</button><button id="vertical-in">+ {_translate("viewer-vertical-word", lang)}</button>
<label>{_translate("viewer-axis-label", lang)} <select id="scale-mode"><option value="major">{_translate("viewer-scale-major", lang)}</option><option value="minor">{_translate("viewer-scale-minor", lang)}</option><option value="nihavent">Nihavend</option><option value="kurdi">Kürdi</option><option value="ussak">Uşşak</option><option value="hicaz">Hicaz</option><option value="kurdilihicazkar">Kürdilihicazkâr</option><option value="hicazkar">Hicazkâr</option><option value="turkish">{_translate("viewer-axis-turkish", lang)}</option></select></label><button id="makam-settings-open" type="button">{_translate("viewer-settings-button", lang)}</button><span id="makam-status" class="countdown-status"></span>
...
<label>{_translate("viewer-countdown-label", lang)} <input id="countdown" type="number" min="0" max="60" step="1" value="0"> {_translate("viewer-seconds-unit", lang)}</label><button id="play-toggle">{_translate("viewer-play-button", lang)}</button><span id="countdown-status" class="countdown-status" aria-live="polite"></span>
<button id="set-a" disabled>A: 0.00 {_translate("viewer-seconds-unit", lang)}</button><button id="set-b" disabled>B: {_translate("viewer-mark-b-end", lang)}</button><button id="loop" aria-pressed="false" disabled>Loop</button><button id="reset">{_translate("viewer-back-to-start", lang)}</button><span class="legend">{_translate("viewer-scroll-legend", lang)}</span>
</div><div id="chart-panel" class="chart-scroll"><canvas id="chart"></canvas><div id="countdown-overlay" class="countdown-overlay" role="status" aria-live="assertive" hidden><div class="countdown-overlay-number"></div></div><input id="vertical-scroll" type="range" aria-label="{_translate('viewer-vertical-scroll-aria', lang)}"><input id="time-scroll" type="range" aria-label="{_translate('viewer-time-scroll-aria', lang)}"><span></span></div><p class="note">{_translate("viewer-scroll-note", lang)}</p><details class="interval-guide"><summary>{_translate("viewer-koma-guide-summary", lang)}</summary><table><thead><tr><th>{_translate("viewer-koma-guide-abbr", lang)}</th><th>{_translate("viewer-koma-guide-interval", lang)}</th><th>{_translate("viewer-koma-guide-koma", lang)}</th><th>{_translate("viewer-koma-guide-notation", lang)}</th></tr></thead><tbody><tr><td>F</td><td>Koma</td><td>1</td><td>♯1 / ♭1</td></tr><tr><td>E</td><td>Eksik bakiye</td><td>3</td><td>—</td></tr><tr><td>B</td><td>Bakiye</td><td>4</td><td>♯4 / ♭4</td></tr><tr><td>S</td><td>Küçük mücennep</td><td>5</td><td>♯5 / ♭5</td></tr><tr><td>K</td><td>Büyük mücennep</td><td>8</td><td>♯8 / ♭8</td></tr><tr><td>T</td><td>Tanini</td><td>9</td><td>♯9 / ♭9</td></tr><tr><td>A</td><td>Artık ikili</td><td>12–13</td><td>—</td></tr></tbody></table></details><span class="resize-handle resize-right" data-resize="right"></span><span class="resize-handle resize-bottom" data-resize="bottom"></span><span class="resize-handle resize-corner" data-resize="corner"></span></section></div><dialog id="makam-settings"><form method="dialog" class="settings-form"><h2>{_translate("viewer-settings-title", lang)}</h2><p>{_translate("viewer-settings-description", lang)}</p><div class="settings-actions" style="justify-content:flex-start;margin-top:0"><label><input id="vertical-follow" type="checkbox" checked> {_translate("viewer-vertical-follow-checkbox", lang)}</label><label>{_translate("viewer-playback-speed-label", lang)} <select id="playback-rate"><option value="0.10">0,10×</option><option value="0.15">0,15×</option><option value="0.20">0,20×</option><option value="0.25">0,25×</option><option value="0.30">0,30×</option><option value="0.35">0,35×</option><option value="0.40">0,40×</option><option value="0.45">0,45×</option><option value="0.50">0,50×</option><option value="0.55">0,55×</option><option value="0.60">0,60×</option><option value="0.65">0,65×</option><option value="0.70">0,70×</option><option value="0.75">0,75×</option><option value="0.80">0,80×</option><option value="0.85">0,85×</option><option value="0.90">0,90×</option><option value="0.95">0,95×</option><option value="1.00" selected>1,00×</option></select></label></div><label>{_translate("viewer-makam-label", lang)} <select id="makam-settings-mode"><option value="nihavent">Nihavend</option><option value="kurdi">Kürdi</option><option value="ussak">Uşşak</option><option value="hicaz">Hicaz</option><option value="kurdilihicazkar">Kürdilihicazkâr</option><option value="hicazkar">Hicazkâr</option></select></label><div id="makam-settings-grid" class="settings-grid"></div><div id="makam-settings-total" class="settings-total"></div><div class="settings-actions"><button id="makam-settings-reset" type="button">{_translate("viewer-reset-to-theory", lang)}</button><button value="cancel">{_translate("viewer-cancel-button", lang)}</button><button id="makam-settings-apply" type="button">{_translate("viewer-apply-button", lang)}</button></div></form></dialog></main><script>
const audioOnly={json.dumps(is_audio_only)};
const frames={json.dumps(frames, ensure_ascii=False, separators=(',', ':'))};
// Birlikte Çal modunun mikrofon eğrisi: {{t,hz}} nesneleri, zamana göre artan.
// Boşken sayfa bugünküyle birebir aynı çizer.
let micFrames=[];
...
const graphAppearanceKey='klarivision-graph-appearance-v1',defaultGraphAppearance={{pitchHex:'#0A84FF',noteGuideHex:'#8E8E93',kararHex:'#E75A5A'}};
// Mikrofon rengi ayrı eklenir: mevcut iki renk sabit kalsın, üçüncüsü aynı nesneye katılır.
defaultGraphAppearance.micHex='#FF9F0A';
let graphAppearanceInputs=null,pendingGraphAppearance=null;
const makamStatus=document.getElementById('makam-status');let contextStatus,playbackRateStatus;
const winInput=document.getElementById('window'),countdownInput=document.getElementById('countdown'),playToggle=document.getElementById('play-toggle'),countdownStatus=document.getElementById('countdown-status'),countdownOverlay=document.getElementById('countdown-overlay');let windowSeconds=12,viewStart=-6,drag=null,followPlayback=true,countdownTimer=null,countdownBypass=false,countdownRemaining=0;
...
function notesForMode(mode,intervals,labelTonic=Number(tonicInput.value),soundingTonic=Number(tonicInput.value)){{const names=scaleLabels[mode][String(labelTonic)],namesByPitchClass=new Map(intervals.map((interval,index)=>[(soundingTonic+interval)%12,names[index]])),result=[];for(let midi=24;midi<=108;midi++){{const name=namesByPitchClass.get(midi%12);if(!name)continue;const octave=Math.floor(midi/12)-1,hz=440*Math.pow(2,(midi-69)/12),isKarar=midi%12===soundingTonic;result.push([`${{name}}${{octave}}`,hz,isKarar])}}return result}}
...
function makamNotes(mode,labelMode){{const solClarinetTonic=Number(tonicInput.value),soundingTonic=(solClarinetTonic+7)%12,tonicMidi=60+soundingTonic,rootKoma=naturalKomaByPitchClass[solClarinetTonic],names=scaleLabels[labelMode][String(solClarinetTonic)],steps=[0];for(const interval of makamIntervals[mode])steps.push(steps.at(-1)+interval);const result=[];for(let octave=-3;octave<=3;octave++){{for(let degree=0;degree<7;degree++){{const hz=440*Math.pow(2,(tonicMidi-69)/12)*Math.pow(2,octave)*Math.pow(2,steps[degree]/53),label=makamLabel(names[degree],Math.floor(tonicMidi/12)-1+octave,steps[degree],rootKoma),isKarar=degree===0;result.push([label,hz,isKarar])}}}}return result}}
...
function turkishNotes(){{return turkishReference.map(note=>[note.display_notation,note.frequency_hz,false])}}
...
function renderMakamSettings(){{const mode=makamSettingsMode.value,values=pendingMakamIntervals[mode],total=values.reduce((sum,value)=>sum+Number(value),0);makamSettingsGrid.replaceChildren(...values.map((value,index)=>{{const label=document.createElement('label'),select=document.createElement('select');label.textContent=`{_translate("viewer-interval-word", lang)} ${{index+1}}`;select.dataset.intervalIndex=index;for(let koma=1;koma<=13;koma++){{const option=document.createElement('option');option.value=koma;option.textContent=komaOption(koma);option.selected=koma===Number(value);select.append(option)}}label.append(select);return label}}));makamSettingsTotal.textContent=`{_translate("viewer-total-koma-prefix", lang)} ${{total}} / 53 koma`;makamSettingsTotal.classList.toggle('invalid',total!==53);makamSettingsApply.disabled=total!==53}}
...
function validGraphColor(value){{return typeof value==='string'&&/^#[0-9a-fA-F]{{6}}$/.test(value)?value.toUpperCase():null}}
function normalizeGraphAppearance(value){{return {{pitchHex:validGraphColor(value?.pitchHex)||defaultGraphAppearance.pitchHex,noteGuideHex:validGraphColor(value?.noteGuideHex)||defaultGraphAppearance.noteGuideHex,micHex:validGraphColor(value?.micHex)||defaultGraphAppearance.micHex,kararHex:validGraphColor(value?.kararHex)||defaultGraphAppearance.kararHex}}}}
let graphAppearance=(()=>{{try{{return normalizeGraphAppearance(JSON.parse(localStorage.getItem(graphAppearanceKey)))}}catch(_error){{return {{...defaultGraphAppearance}}}}}})();
function colorWithAlpha(hex,alpha){{const value=validGraphColor(hex)||defaultGraphAppearance.noteGuideHex,red=parseInt(value.slice(1,3),16),green=parseInt(value.slice(3,5),16),blue=parseInt(value.slice(5,7),16);return `rgba(${{red}},${{green}},${{blue}},${{alpha}})`}}
function setGraphAppearance(value,{{persist=true,notify=true}}={{}}){{graphAppearance=normalizeGraphAppearance(value);if(persist)localStorage.setItem(graphAppearanceKey,JSON.stringify(graphAppearance));if(graphAppearanceInputs){{graphAppearanceInputs.pitch.value=graphAppearance.pitchHex;graphAppearanceInputs.noteGuide.value=graphAppearance.noteGuideHex;graphAppearanceInputs.mic.value=graphAppearance.micHex;graphAppearanceInputs.karar.value=graphAppearance.kararHex}}if(notify)window.webkit?.messageHandlers?.graphAppearance?.postMessage(graphAppearance);draw();return graphAppearance}}
function chartPalette(){{if(document.body.classList.contains('theme-studio'))return {{background:'#20262d',grid:'#3b4651',axis:'#bdcad5',label:'#e7f3f7',curve:graphAppearance.pitchHex,guide:graphAppearance.noteGuideHex,mic:graphAppearance.micHex,karar:graphAppearance.kararHex,frame:'#657888',loop:'rgba(51,184,199,.23)',playhead:'#d99aff'}};if(document.body.classList.contains('theme-classic'))return {{background:'#fffaf1',grid:'#eadfce',axis:'#a38b69',label:'#263647',curve:graphAppearance.pitchHex,guide:graphAppearance.noteGuideHex,mic:graphAppearance.micHex,karar:graphAppearance.kararHex,frame:'#c7a66d',loop:'rgba(204,162,81,.20)',playhead:'#9a6ab0'}};return {{background:'#fff',grid:'#e2e6eb',axis:'#edf0f3',label:'#3d4854',curve:graphAppearance.pitchHex,guide:graphAppearance.noteGuideHex,mic:graphAppearance.micHex,karar:graphAppearance.kararHex,frame:'#aeb8c3',loop:'rgba(112,181,235,.20)',playhead:'#7755b8'}}}}
function setPlaybackRate(value){{const rate=Math.max(.10,Math.min(2,Math.round(value*20)/20)),key=rate.toFixed(2);if(![...playbackRateInput.options].some(option=>option.value===key)){{const option=document.createElement('option');option.value=key;option.textContent=`${{key.replace('.',',')}}×`;playbackRateInput.append(option)}}playbackRateInput.value=key;media.defaultPlaybackRate=rate;media.playbackRate=rate;if(playbackRateStatus)playbackRateStatus.textContent=`${{key.replace('.',',')}}×`}}
function addAppearanceSettings(){{const form=makamSettingsDialog.querySelector('.settings-form'),section=document.createElement('section'),label=document.createElement('label'),select=document.createElement('select'),heading=document.createElement('h3'),controls=document.createElement('div'),controlsRow=playToggle.parentElement,time=document.createElement('span'),loopLabel=document.createElement('span');section.className='settings-section settings-appearance';heading.textContent='{_translate_js("viewer-appearance-heading", lang)}';label.textContent='{_translate_js("viewer-theme-label", lang)}';select.id='appearance-theme';[['focus','{_translate_js("viewer-theme-focus", lang)}'],['studio','{_translate_js("viewer-theme-studio", lang)}'],['classic','{_translate_js("viewer-theme-classic", lang)}']].forEach(([value,text])=>{{const option=document.createElement('option');option.value=value;option.textContent=text;select.append(option)}});label.append(select);section.append(heading,label);form.insertBefore(section,form.firstChild);appearanceThemeInput=select;select.onchange=()=>setAppearance(select.value);controls.className='player-controls';controls.setAttribute('aria-label','{_translate_js("viewer-transport-aria", lang)}');setAButton.className='transport-jump';setBButton.className='transport-jump';setAButton.textContent='|◀';setBButton.textContent='▶|';setAButton.title='{_translate_js("viewer-mark-a-title", lang)}';setBButton.title='{_translate_js("viewer-mark-b-title", lang)}';time.className='transport-time';timeReadout=time;loopLabel.className='loop-label';loopLabel.textContent='Loop';loopButton.textContent='';loopButton.title='{_translate_js("viewer-loop-toggle-title", lang)}';controls.append(playToggle,setAButton,setBButton,time,loopLabel,loopButton);controlsRow.append(controls);setAppearance(activeAppearance())}}
function addGraphAppearanceSettings(){{const form=makamSettingsDialog.querySelector('.settings-form'),section=document.createElement('section'),heading=document.createElement('h3'),row=document.createElement('div'),pitchLabel=document.createElement('label'),pitch=document.createElement('input'),guideLabel=document.createElement('label'),noteGuide=document.createElement('input'),micLabel=document.createElement('label'),mic=document.createElement('input'),kararLabel=document.createElement('label'),karar=document.createElement('input'),reset=document.createElement('button');section.className='settings-section graph-appearance-settings';section.id='graph-appearance-settings';heading.textContent='{_translate_js("viewer-graph-colors-heading", lang)}';row.className='settings-row graph-color-row';pitch.type='color';pitch.id='graph-pitch-color';pitchLabel.textContent='{_translate_js("viewer-pitch-curve-label", lang)}';pitchLabel.append(pitch);noteGuide.type='color';noteGuide.id='graph-note-guide-color';guideLabel.textContent='{_translate_js("viewer-note-guides-label", lang)}';guideLabel.append(noteGuide);mic.type='color';mic.id='graph-mic-color';micLabel.textContent='{_translate_js("viewer-mic-curve-label", lang)}';micLabel.append(mic);karar.type='color';karar.id='graph-karar-color';kararLabel.textContent='{_translate_js("viewer-karar-tone-label", lang)}';kararLabel.append(karar);reset.type='button';reset.id='graph-appearance-reset';reset.textContent='{_translate_js("viewer-reset-default-colors", lang)}';row.append(pitchLabel,guideLabel,micLabel,kararLabel);section.append(heading,row,reset);const appearanceSection=form.querySelector('.settings-appearance');appearanceSection.after(section);graphAppearanceInputs={{pitch,noteGuide,mic,karar}};reset.onclick=()=>{{pendingGraphAppearance={{...defaultGraphAppearance}};pitch.value=pendingGraphAppearance.pitchHex;noteGuide.value=pendingGraphAppearance.noteGuideHex;mic.value=pendingGraphAppearance.micHex;karar.value=pendingGraphAppearance.kararHex}}}}
function organisePracticeControls(){{const form=makamSettingsDialog.querySelector('.settings-form'),makamLabel=makamSettingsMode.closest('label'),speedRow=playbackRateInput.closest('.settings-actions'),speedLabel=playbackRateInput.closest('label'),practice=document.createElement('section'),practiceRow=document.createElement('div'),speedStepper=document.createElement('div'),speedDown=document.createElement('button'),speedUp=document.createElement('button'),title=document.querySelector('h1'),description=document.querySelector('main > p'),deferredControls=[document.getElementById('minus'),document.getElementById('plus'),winInput.closest('label'),document.getElementById('vertical-out'),document.getElementById('vertical-in')];practice.className='settings-section';practiceRow.className='settings-row';speedStepper.className='speed-stepper';speedDown.type='button';speedDown.textContent='−';speedUp.type='button';speedUp.textContent='+';playbackRateStatus=document.createElement('span');speedStepper.append(speedDown,playbackRateStatus,speedUp);speedLabel.hidden=true;speedRow.append(speedStepper);speedDown.onclick=()=>setPlaybackRate(Number(playbackRateInput.value)-.05);speedUp.onclick=()=>setPlaybackRate(Number(playbackRateInput.value)+.05);practice.innerHTML='<h3>{_translate("viewer-study-heading", lang)}</h3>';practiceRow.append(layoutMode.closest('label'),scaleMode.closest('label'),tonicInput.closest('label'),countdownInput.closest('label'));practice.append(practiceRow);form.insertBefore(practice,speedRow);makamLabel.before(makamStatus);deferredControls.forEach(control=>control.hidden=true);document.querySelector('.layout-tools').remove();title.textContent='KlariVision';contextStatus=document.createElement('span');contextStatus.id='context-status';contextStatus.className='app-context';title.after(contextStatus);description.textContent='{_translate_js("viewer-frequency-description", lang)}';setPlaybackRate(Number(playbackRateInput.value))}}
makamSettingsOpen.onclick=()=>{{pendingMakamIntervals=JSON.parse(JSON.stringify(makamIntervals));pendingGraphAppearance={{...graphAppearance}};if(graphAppearanceInputs){{graphAppearanceInputs.pitch.value=pendingGraphAppearance.pitchHex;graphAppearanceInputs.noteGuide.value=pendingGraphAppearance.noteGuideHex;graphAppearanceInputs.mic.value=pendingGraphAppearance.micHex;graphAppearanceInputs.karar.value=pendingGraphAppearance.kararHex}}if(makamIntervals[scaleMode.value])makamSettingsMode.value=scaleMode.value;renderMakamSettings();makamSettingsDialog.showModal()}};makamSettingsMode.onchange=renderMakamSettings;makamSettingsGrid.onchange=event=>{{if(!event.target.matches('select[data-interval-index]'))return;pendingMakamIntervals[makamSettingsMode.value][Number(event.target.dataset.intervalIndex)]=Number(event.target.value);renderMakamSettings()}};makamSettingsReset.onclick=()=>{{pendingMakamIntervals[makamSettingsMode.value]=[...makamDefaults[makamSettingsMode.value]];renderMakamSettings()}};makamSettingsApply.onclick=()=>{{const values=pendingMakamIntervals[makamSettingsMode.value];if(values.reduce((sum,value)=>sum+Number(value),0)!==53)return;makamIntervals=pendingMakamIntervals;localStorage.setItem(makamSettingsKey,JSON.stringify(makamIntervals));setGraphAppearance({{pitchHex:graphAppearanceInputs?.pitch.value,noteGuideHex:graphAppearanceInputs?.noteGuide.value,micHex:graphAppearanceInputs?.mic.value,kararHex:graphAppearanceInputs?.karar.value}});makamSettingsDialog.close();updateMakamStatus();draw()}};
function formatTime(time){{return `${{time.toFixed(2)}} {_translate_js("viewer-seconds-unit", lang)}`}}
function formatClockMs(time){{const t=Math.max(0,Number(time)||0),minutes=Math.floor(t/60),seconds=Math.floor(t%60),ms=Math.floor((t-Math.floor(t))*1000);return `${{minutes}}:${{String(seconds).padStart(2,'0')}}.${{String(ms).padStart(3,'0')}}`}}
...
window.klariVisionStudyViewer={{
    ...
    scale: scaleMode,
    tonic: tonicInput,
    follow: verticalFollowInput,
    setGraphAppearance,
    settingsSnapshot(){{
        return {{theme:activeAppearance(),scale:scaleMode.value,tonic:Number(tonicInput.value),countdown:Number(countdownInput.value)||0,intervals:makamIntervals,graphAppearance:{{...graphAppearance}}}};
    }},
    applySettings(value){{
        if(!value||typeof value!=='object')return false;
        const intervals=value.intervals;
        if(!intervals||!Object.keys(makamDefaults).every(mode=>Array.isArray(intervals[mode])&&intervals[mode].length===7&&intervals[mode].every(item=>Number.isInteger(item)&&item>=1&&item<=13)&&intervals[mode].reduce((sum,item)=>sum+item,0)===53))return false;
        makamIntervals=JSON.parse(JSON.stringify(intervals));
        localStorage.setItem(makamSettingsKey,JSON.stringify(makamIntervals));
        setAppearance(value.theme);
        setGraphAppearance(value.graphAppearance);
        if(scaleMode.querySelector(`option[value="${{value.scale}}"]`)){{scaleMode.value=value.scale;scaleMode.dispatchEvent(new Event('change'));}}
        if(tonicInput.querySelector(`option[value="${{Number(value.tonic)}}"]`)){{tonicInput.value=String(Number(value.tonic));tonicInput.dispatchEvent(new Event('change'));}}
        countdownInput.value=String(Math.max(0,Math.min(60,Math.round(Number(value.countdown)||0))));
        updateMakamStatus();draw();
        return true;
    }},
    snapshot(){{
        const time=Number(media.currentTime)||0;
        let nearest=null,delta=Infinity;
        for(const frame of frames){{const distance=Math.abs(frame.t-time);if(distance<delta){{nearest=frame;delta=distance}}}}
        return {{time,duration:Number.isFinite(media.duration)?media.duration:duration,ready:mediaReady,isPlaying:!media.paused,rate:Number(media.playbackRate)||1,loopEnabled,loopA,loopB,theme:activeAppearance(),followsCurve:verticalFollowInput.checked,frequency:nearest&&delta<=.15?nearest.hz:null,scale:scaleMode.value,tonic:Number(tonicInput.value),intervals:makamIntervals[scaleMode.value]||null,muted:!!media.muted,countdown:countdownTimer?countdownRemaining:0}};
    }},
    micAppend(points){{
        if(!Array.isArray(points)||!points.length)return;
        // Bir mikrofon noktasi dogasi geregi calma basinin GERISINDEDIR: zaten
        // olmus bir sesi anlatir ve Swift sabit gecikmeyi ayrica duser. Calma
        // basinin belirgin olcude ONUNDE gelen nokta yalnizca su durumda
        // uretilir: arama ya da loop B->A donusu olmus, ama Swift'in sunum
        // saati (50 ms'lik anlik goruntuyle beslendigi icin) sicramayi henuz
        // gormemis; o kare hala B civarina damgalaniyor. Bu BAYAT nokta
        // alinirsa `lastT` B'ye kacar ve donusten sonraki gercek noktalarin
        // hepsi asagidaki `t<lastT` elemesine takilir -> o loop turu hic
        // cizilmez. Yaris her turda yeniden atildigi icin belirti "3-4 loop'ta
        // bir cizmiyor" seklinde gorunur. Tolerans sayfanin kendi sicrama
        // esigiyle ayni (0,35 sn); kullanici hizalamasi +/-200 ms oldugundan
        // gercek noktalar bu ufku asamaz.
        const horizon=(media.currentTime||0)+.35;
        let lastT=micFrames.length?micFrames[micFrames.length-1].t:-Infinity;
        for(const point of points){{
            if(!point||typeof point!=='object')continue;
            const t=Number(point.t),hz=Number(point.hz);
            if(!Number.isFinite(t)||t<lastT||t>horizon)continue;
    ...
    micClear(){{micFrames=[];draw();}},
    micTruncate(time){{const cutoff=micIndexAtOrAfter(Number(time)||0);micFrames.length=cutoff;draw();}},
    setMicColor(hex){{setGraphAppearance({{...graphAppearance,micHex:hex}});}},
    setMuted(flag){{media.muted=!!flag;return media.muted;}},
    command(value){{
        if(value&&typeof value==='object'&&value.type==='seek'){{media.currentTime=Math.max(0,Math.min(Number(value.seconds)||0,duration));followPlayback=true;centerOnPlayhead();draw();return}}
        if(value==='toggle'){{playToggle.click();return}}
        if(value==='reset'){{document.getElementById('reset').click();return}}
        if(value==='setA'){{setAButton.click();return}}
        if(value==='setB'){{setBButton.click();return}}
        if(value==='loop'){{loopButton.click();return}}
        if(value==='speedDown'){{setPlaybackRate(Number(media.playbackRate||playbackRateInput.value)-.05);return}}
        if(value==='speedUp'){{setPlaybackRate(Number(media.playbackRate||playbackRateInput.value)+.05);return}}
        if(value==='follow'){{verticalFollowInput.checked=!verticalFollowInput.checked;verticalFollowInput.dispatchEvent(new Event('change',{{bubbles:true}}));}}
    }}
}};
...
// Sunum saati.  Pencere ve playhead doğrudan media.currentTime'a bağlanınca
// grafik medya saatinin donmalarını birebir gösteriyor: ölçümde oynatmanın
// ilk saniyesinde ve her arama/loop dönüşünde currentTime 130-200 ms boyunca
// hiç ilerlemiyor, sonra tek karede toparlıyor.  Bunun yerine saat kendi
// hızıyla akar ve medya saatine yalnızca süzülerek yaklaşır; gerçek atlamalar
// (arama, loop başa dönüşü) 0,35 sn eşiğini aştığı için anında yakalanır.
// Aynı sözleşme Çalma Modu grafiğinde de var (LiveVisuals.swift).
let shownTime=0,shownAt=performance.now();
function playbackTime(){{return shownTime}}
function snapClock(){{shownTime=Number(media.currentTime)||0;shownAt=performance.now()}}
function advanceClock(){{const wall=performance.now(),goal=Number(media.currentTime)||0;if(media.paused||!Number.isFinite(goal)){{shownTime=goal;shownAt=wall;return shownTime}}const step=(wall-shownAt)/1000*(Number(media.playbackRate)||1);shownAt=wall;const flowed=shownTime+step,error=goal-flowed;if(Math.abs(error)>.35){{shownTime=goal;return shownTime}}
// Düzeltme, o karenin adımının bir kısmıyla sınırlı: saat asla geri gitmez ve
// farkı kapatırken hız gözle görülür biçimde değişmez.
shownTime=flowed+Math.max(-step*.75,Math.min(step*.75,error*.05));return shownTime}}
function frameIndexAtOrAfter(time){{let lo=0,hi=frames.length;while(lo<hi){{const mid=(lo+hi)>>1;if(frames[mid].t<time)lo=mid+1;else hi=mid}}return lo}}
function frameRangeFor(start,end){{return [frameIndexAtOrAfter(start),frameIndexAtOrAfter(end+1e-9)]}}
// micFrames on binlerce nokta olabilir; frameIndexAtOrAfter ile aynı ikili
// arama kalıbı, görünür pencereyi tüm diziyi taramadan bulur.
function micIndexAtOrAfter(time){{let lo=0,hi=micFrames.length;while(lo<hi){{const mid=(lo+hi)>>1;if(micFrames[mid].t<time)lo=mid+1;else hi=mid}}return lo}}
function micRangeFor(start,end){{return [micIndexAtOrAfter(start),micIndexAtOrAfter(end+1e-9)]}}
function visibleValues(){{const [from,to]=frameRangeFor(viewStart,viewStart+windowSeconds),values=[];for(let i=from;i<to;i++)values.push(cents(frames[i].hz));return values}}
...
function followValues(){{const time=playbackTime(),[from,to]=frameRangeFor(time-.25,time+.25),values=[];for(let i=from;i<to;i++)values.push(cents(frames[i].hz));return values}}
function updateVerticalFollow(){{if(!verticalFollowInput.checked)return;const target=followValues();if(!target.length)return;const pitch=middle(target),edge=verticalSpan*.45,restingEdge=verticalSpan*.35;if(pitch>verticalCenter+edge)verticalCenter=pitch-restingEdge;else if(pitch<verticalCenter-edge)verticalCenter=pitch+restingEdge}}
function range(){{if(!verticalReady){{const values=visibleValues();verticalCenter=middle(values.length?values:frames.map(p=>cents(p.hz)));verticalReady=true}}updateVerticalFollow();return [verticalCenter-verticalSpan/2,verticalCenter+verticalSpan/2]}}
let frameCentsBounds=null;
...
ctx.save();ctx.beginPath();ctx.rect(left,marginTop,cw,ch);ctx.clip();ctx.strokeStyle=palette.curve;ctx.lineWidth=1.7;ctx.lineJoin='round';ctx.lineCap='round';ctx.beginPath();let previous=null;const [curveFrom,curveTo]=frameRangeFor(viewStart-.05,viewStart+windowSeconds+.05);for(let i=curveFrom;i<curveTo;i++){{const p=frames[i],xx=x(p.t),yy=y(p.hz);if(!previous||p.t-previous.t>.040)ctx.moveTo(xx,yy);else ctx.lineTo(xx,yy);previous=p}}ctx.stroke();
// Mikrofon eğrisi: aynı x()/y() geometrisi, görünür pencereye ikili aramayla
// düşen noktalar. 0,12 sn'yi aşan boşluklarda yol kırılır (loop/seek/geri sıçrama).
if(micFrames.length){{ctx.strokeStyle=palette.mic;ctx.lineWidth=1.7;ctx.lineJoin='round';ctx.lineCap='round';ctx.beginPath();let micPrevious=null;const [micFrom,micTo]=micRangeFor(viewStart-.05,viewStart+windowSeconds+.05);for(let i=micFrom;i<micTo;i++){{const p=micFrames[i],xx=x(p.t),yy=y(p.hz);if(!micPrevious||p.t-micPrevious.t>.12)ctx.moveTo(xx,yy);else ctx.lineTo(xx,yy);micPrevious=p}}ctx.stroke()}}
function marker(time,label,color){{if(time<viewStart||time>viewStart+windowSeconds)return;const xx=x(time);ctx.strokeStyle=color;ctx.lineWidth=1.5;ctx.beginPath();ctx.moveTo(xx,marginTop);ctx.lineTo(xx,h-bottom);ctx.stroke();ctx.fillStyle=color;ctx.font='bold 12px system-ui';ctx.textAlign='center';ctx.fillText(label,xx,marginTop+14)}}marker(loopA,'A','#157347');marker(loopB,'B','#bd5b00');const current=playbackTime();if(current>=viewStart&&current<=viewStart+windowSeconds){{const xx=x(current);ctx.strokeStyle=palette.playhead;ctx.lineWidth=1.5;ctx.beginPath();ctx.moveTo(xx,marginTop);ctx.lineTo(xx,h-bottom);ctx.stroke()}}ctx.restore();ctx.strokeStyle=palette.frame;ctx.strokeRect(left,marginTop,cw,ch);ctx.fillStyle=palette.label;ctx.font='600 12px ui-monospace,SFMono-Regular,Menlo,monospace';ctx.textAlign='right';ctx.textBaseline='top';ctx.fillText(formatClockMs(current),w-right,4);ctx.textBaseline='alphabetic';updateScrollbars()}}
function clampStart(){{viewStart=Math.max(-windowSeconds/2,Math.min(viewStart,duration-windowSeconds/2))}}
function centerOnPlayhead(){{viewStart=playbackTime()-windowSeconds/2;clampStart()}}
function loopStart(){{return Math.max(0,Math.min(loopA,Math.max(0,duration-.001)))}}
function restartLoopAtA(){{media.currentTime=loopStart();const seconds=countdownSeconds();if(seconds>0&&!countdownTimer){{media.pause();runCountdown(seconds,()=>{{countdownBypass=true;media.play().catch(()=>{{}})}})}}}}
function playLoopFromA(){{const start=loopStart(),seconds=countdownSeconds();if(seconds>0){{countdownBypass=false;media.pause();media.currentTime=start;if(countdownTimer)return;runCountdown(seconds,()=>{{countdownBypass=true;media.play().then(()=>{{if(loopEnabled&&Math.abs((media.currentTime||0)-start)>.05)media.currentTime=start}}).catch(()=>{{}})}});return}}media.play().then(()=>{{if(loopEnabled)media.currentTime=start}}).catch(()=>{{}})}}
function tick(){{if(loopEnabled&&!media.paused&&(media.currentTime||0)>=loopB)restartLoopAtA();
// Geriye sıçrama tespiti: loop B→A dönüşü ve zamanda geriye sürüklemenin
// ikisi de aynı kuralla yakalanır — mikrofon eğrisi artık kalmış noktaları
// yeni geçişten önce budanır. Ayrı bir loop kuralı yok, kasıtlı.
const now=media.currentTime||0;if(now<lastObservedMediaTime-.05)window.klariVisionStudyViewer.micTruncate(now);lastObservedMediaTime=now;
advanceClock();if(followPlayback)centerOnPlayhead();if(timeReadout){{const text=`${{formatClock(playbackTime())}} / ${{formatClock(duration)}}`;if(timeReadout.textContent!==text)timeReadout.textContent=text}}draw();requestAnimationFrame(tick)}}
...
function updatePlayButton(){{playToggle.textContent=countdownTimer?'{_translate_js("viewer-cancel-countdown", lang)}':(media.paused?'{_translate_js("viewer-play-button", lang)}':'{_translate_js("viewer-pause-button", lang)}');playToggle.classList.toggle('is-playing',!media.paused&&!countdownTimer)}}
// Geri sayım hem Oynat'ta hem A-B döngüsünün her başa dönüşünde çalışır.
// Sayı grafiğin üstünde büyük gösterilir; uygulama kabuğu sayfanın kontrol
// satırını gizlediği için yalnız `countdown-status` yazısı görünmezdi.
// `countdownRemaining` snapshot() ile yerel oynatma çubuğuna da gider.
function showCountdown(value){{countdownRemaining=value;countdownStatus.textContent=value>0?`{_translate("viewer-starting-countdown-prefix", lang)} ${{value}}`:'';if(countdownOverlay){{countdownOverlay.hidden=value<=0;countdownOverlay.firstElementChild.textContent=value>0?String(value):''}}}}
function clearCountdown(){{if(countdownTimer){{clearInterval(countdownTimer);countdownTimer=null}}showCountdown(0);updatePlayButton()}}
function countdownSeconds(){{return Math.max(0,Math.floor(Number(countdownInput.value)||0))}}
function runCountdown(seconds,done){{if(countdownTimer)clearInterval(countdownTimer);let remaining=seconds;showCountdown(remaining);countdownTimer=setInterval(()=>{{remaining-=1;if(remaining<=0){{clearCountdown();done()}}else showCountdown(remaining)}},1000);updatePlayButton()}}
function beginPlayback(){{if(!media.paused){{media.pause();return}}const seconds=countdownSeconds();if(seconds===0){{countdownBypass=true;media.play().catch(()=>{{}});return}}runCountdown(seconds,()=>{{countdownBypass=true;media.play().catch(()=>{{}})}})}}
...
function installTooltips(){{const copy={{minus:'{_translate_js("viewer-tooltip-zoom-in", lang)}',plus:'{_translate_js("viewer-tooltip-zoom-out", lang)}',window:'{_translate_js("viewer-tooltip-window-seconds", lang)}','vertical-out':'{_translate_js("viewer-tooltip-vertical-expand", lang)}','vertical-in':'{_translate_js("viewer-tooltip-vertical-shrink", lang)}','scale-mode':'{_translate_js("viewer-tooltip-scale-mode", lang)}','tonic':'{_translate_js("viewer-tooltip-tonic", lang)}',countdown:'{_translate_js("viewer-tooltip-countdown", lang)}','play-toggle':'{_translate_js("viewer-tooltip-play-toggle", lang)}',reset:'{_translate_js("viewer-tooltip-reset", lang)}','time-scroll':'{_translate_js("viewer-tooltip-time-scroll", lang)}','vertical-scroll':'{_translate_js("viewer-tooltip-vertical-scroll", lang)}','makam-settings-open':'{_translate_js("viewer-tooltip-makam-settings", lang)}'}};const bubble=document.createElement('div');bubble.className='klarivision-tooltip';document.body.append(bubble);let timer=null;const hide=()=>{{clearTimeout(timer);bubble.hidden=true}};Object.entries(copy).forEach(([id,text])=>{{const element=document.getElementById(id);if(!element)return;element.setAttribute('aria-label',element.getAttribute('aria-label')||text);element.addEventListener('mouseenter',()=>{{timer=setTimeout(()=>{{bubble.textContent=text;bubble.hidden=false;const rect=element.getBoundingClientRect();bubble.style.left=`${{Math.max(8,rect.left)}}px`;bubble.style.top=`${{Math.max(8,rect.bottom+6)}}px`}},500)}});element.addEventListener('mouseleave',hide);element.addEventListener('mousemove',hide);element.addEventListener('focus',()=>{{bubble.textContent=text;bubble.hidden=false;const rect=element.getBoundingClientRect();bubble.style.left=`${{Math.max(8,rect.left)}}px`;bubble.style.top=`${{Math.max(8,rect.bottom+6)}}px`}});element.addEventListener('blur',hide)}});document.addEventListener('keydown',event=>{{if(event.key==='Escape')hide()}});const style=document.createElement('style');style.textContent='.klarivision-tooltip{{position:fixed;z-index:9999;max-width:260px;padding:6px 8px;border:1px solid rgba(116,135,152,.35);border-radius:7px;background:#202a33;color:#fff;font:12px system-ui;box-shadow:0 4px 14px rgba(20,43,66,.24);pointer-events:none}}.klarivision-tooltip[hidden]{{display:none!important}}';document.head.append(style)}}
const standaloneSettingsStyle=document.createElement('style');standaloneSettingsStyle.textContent='dialog{{max-height:calc(100vh - 28px);overflow-y:auto}}';document.head.append(standaloneSettingsStyle);
organisePracticeControls();addAppearanceSettings();addGraphAppearanceSettings();installTooltips();document.getElementById('minus').onclick=()=>setWindow(windowSeconds*1.35);document.getElementById('plus').onclick=()=>setWindow(windowSeconds/1.35);document.getElementById('reset').onclick=()=>{{media.currentTime=0;followPlayback=true;centerOnPlayhead();draw()}};winInput.onchange=()=>setWindow(Number(winInput.value));updateMakamStatus();
