...
VIEWER_VERSION = "0.4.4-makam-ayarlari"
...
makamSettingsOpen.onclick=()=>{{pendingMakamIntervals=JSON.parse(JSON.stringify(makamIntervals));renderMakamSettings();makamSettingsDialog.showModal()}};makamSettingsMode.onchange=renderMakamSettings;makamSettingsGrid.onchange=event=>{{if(!event.target.matches('select[data-interval-index]'))return;pendingMakamIntervals[makamSettingsMode.value][Number(event.target.dataset.intervalIndex)]=Number(event.target.value);renderMakamSettings()}};makamSettingsReset.onclick=()=>{{pendingMakamIntervals[makamSettingsMode.value]=[...makamDefaults[makamSettingsMode.value]];renderMakamSettings()}};makamSettingsApply.onclick=()=>{{const values=pendingMakamIntervals[makamSettingsMode.value];if(values.reduce((sum,value)=>sum+Number(value),0)!==53)return;makamIntervals=pendingMakamIntervals;localStorage.setItem(makamSettingsKey,JSON.stringify(makamIntervals));makamSettingsDialog.close();draw()}};
