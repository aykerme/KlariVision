...
<style>
:root{{color-scheme:light}}*{{box-sizing:border-box}}body{{margin:0;background:#f5f6f8;color:#17212b;font:14px system-ui,-apple-system,sans-serif}}main{{max-width:1320px;margin:auto;padding:22px}}h1{{font-size:21px;margin:0 0 4px}}p{{margin:0 0 16px;color:#56616e}}.layout-tools{{display:flex;align-items:center;gap:12px;flex-wrap:wrap;margin:0 0 14px}}.layout-tools input{{width:130px}}.workspace{{--media-width:660px;--chart-height:580px;display:grid;gap:16px}}.workspace.side{{grid-template-columns:minmax(280px,var(--media-width)) minmax(0,1fr);align-items:start}}.workspace.side-right .media{{order:2}}.media{{position:sticky;top:0;background:#f5f6f8;padding:10px 0 14px;z-index:2;max-width:var(--media-width)}}.workspace.side .media{{max-width:none}}video,audio{{display:block;max-width:100%;width:100%;max-height:520px}}.panel{{min-width:0;background:#fff;border:1px solid #dbe0e6;border-radius:12px;padding:14px}}.tools{{display:flex;align-items:center;gap:12px;flex-wrap:wrap;margin-bottom:10px}}button{{border:1px solid #b9c3cf;background:#fff;border-radius:7px;padding:6px 10px;font:inherit;cursor:pointer}}button:hover{{background:#eef5fb}}button[aria-pressed="true"]{{background:#dceefe;border-color:#4785be;color:#173d62;box-shadow:inset 0 0 0 1px #8bb7de}}input{{width:100px}}select{{font:inherit;padding:5px 7px;border:1px solid #b9c3cf;border-radius:7px;background:#fff}}.countdown-status{{min-width:76px;color:#7755b8;font-weight:700}}.chart-scroll{{display:grid;grid-template-columns:minmax(0,1fr) 18px;grid-template-rows:var(--chart-height) 18px;gap:5px}}canvas{{display:block;width:100%;height:100%;border:1px solid #dbe0e6;border-radius:8px;touch-action:none}}#time-scroll{{grid-column:1;grid-row:2;width:100%;margin:0;accent-color:#7755b8}}#vertical-scroll{{grid-column:2;grid-row:1;width:18px;height:100%;margin:0;writing-mode:vertical-lr;direction:rtl;accent-color:#7755b8}}.note{{font-size:12px;color:#66717f}}.legend{{margin-left:auto;color:#56616e;font-size:12px}}@media(max-width:850px){{.workspace.side{{grid-template-columns:1fr}}.workspace.side-right .media{{order:0}}.media{{max-width:100%}}}}@media(max-width:650px){{main{{padding:12px}}.workspace{{--chart-height:470px}}}}
</style><main>
...
<div class="layout-tools"><label>Yerleşim <select id="layout-mode"><option value="stacked">Üst üste</option><option value="side">Yan yana · video solda</option><option value="side-right">Yan yana · video sağda</option></select></label><label>Video genişliği <input id="media-width" type="range" min="320" max="920" step="10" value="660"> <output id="media-width-value">660 px</output></label><label>Grafik yüksekliği <input id="chart-height" type="range" min="360" max="900" step="10" value="580"> <output id="chart-height-value">580 px</output></label></div>
<div id="workspace" class="workspace"><div class="media">{media}</div>
...
const workspace=document.getElementById('workspace'),layoutMode=document.getElementById('layout-mode'),mediaWidthInput=document.getElementById('media-width'),chartHeightInput=document.getElementById('chart-height'),mediaWidthValue=document.getElementById('media-width-value'),chartHeightValue=document.getElementById('chart-height-value');
...
function applyLayout(){{workspace.className=`workspace ${{layoutMode.value}}`;workspace.style.setProperty('--media-width',`${{mediaWidthInput.value}}px`);workspace.style.setProperty('--chart-height',`${{chartHeightInput.value}}px`);mediaWidthValue.value=`${{mediaWidthInput.value}} px`;mediaWidthValue.textContent=mediaWidthValue.value;chartHeightValue.value=`${{chartHeightInput.value}} px`;chartHeightValue.textContent=chartHeightValue.value;requestAnimationFrame(resize)}}
