...
VIEWER_VERSION = "0.5.0-calisma-odakli"
...
.app-context{{display:inline-block;margin:0 0 14px;color:#405465;font-size:13px;font-weight:650}}.settings-section{{display:grid;gap:10px;padding:13px 0;border-top:1px solid #e2e7ec}}.settings-section h3{{margin:0;font-size:14px}}.settings-row{{display:flex;align-items:center;gap:10px;flex-wrap:wrap}}.tools #makam-settings-open{{margin-left:auto}}@media(max-width:650px){{.tools #makam-settings-open{{margin-left:0}}}}
...
const makamStatus=document.getElementById('makam-status');let contextStatus;
...
function organisePracticeControls(){{const form=makamSettingsDialog.querySelector('.settings-form'),makamLabel=makamSettingsMode.closest('label'),speedRow=playbackRateInput.closest('.settings-actions'),practice=document.createElement('section'),graph=document.createElement('section'),practiceRow=document.createElement('div'),graphRow=document.createElement('div'),title=document.querySelector('h1'),description=document.querySelector('main > p');practice.className='settings-section';graph.className='settings-section';practiceRow.className='settings-row';graphRow.className='settings-row';practice.innerHTML='<h3>Çalışma</h3>';graph.innerHTML='<h3>Grafik görünümü</h3>';practiceRow.append(layoutMode.closest('label'),scaleMode.closest('label'),tonicInput.closest('label'),countdownInput.closest('label'));graphRow.append(document.getElementById('minus'),document.getElementById('plus'),winInput.closest('label'),document.getElementById('vertical-out'),document.getElementById('vertical-in'));practice.append(practiceRow);graph.append(graphRow);form.insertBefore(practice,speedRow);form.insertBefore(graph,makamLabel);makamLabel.before(makamStatus);document.querySelector('.layout-tools').remove();title.textContent='KlariVision';contextStatus=document.createElement('span');contextStatus.id='context-status';contextStatus.className='app-context';title.after(contextStatus);description.textContent='Pitch eğrisi duyulan fiziksel frekansı (Hz) gösterir.'}}
...
scaleMode.onchange=()=>{{tonicInput.disabled=scaleMode.value==='turkish';updateMakamStatus();draw()}};tonicInput.onchange=()=>{{updateMakamStatus();draw()}};playbackRateInput.onchange=()=>{{media.playbackRate=Number(playbackRateInput.value)||1}};
